...
class ApiCallOptions extends Equatable {
  ...
  ApiCallOptions clone() => ApiCallOptions(
        ...
        callType: callType,
        apiUrl: apiUrl,
        headers: _cloneStringMap(headers),
  ...
  static Map<String, String> _cloneStringMap(Map<String, dynamic> map) {
    try {
      return json.decode(json.encode(map)) as Map<String, String>;
    } catch (_) {
      return Map.from(map);
    }
  }
...
class ApiManager {
  ...
  static Future<ApiCallResponse> requestWithBody(
    ...
    try {
      ...
      log('Error in requestWithBody: $e');
      return ApiCallResponse(
        null,
        {},
        -1,
  ...
  static Future<ApiCallResponse> multipartRequest(
    ...
    try {
      ...
      log('Error in multipartRequest: $e');
      return ApiCallResponse(
        null,
        {},
        -1,
  ...
  static Future<ApiCallResponse> makeApiCall({
    ...
    try {
      ...
      late final ApiCallResponse response;
      if (bodyType == BodyType.MULTIPART) {
        response = await multipartRequest(
          ...
          (callType == ApiCallType.GET || callType == ApiCallType.DELETE)) {
      ...
      if (response.statusCode != 200) {
        log('API call failed: ${response.statusCode} ${response.jsonBody}');
      }

      _apiCache[cacheKey] = response;
      return response;
    } catch (e) {
      log('Error in makeApiCall: $e');
      return ApiCallResponse(
        null,
        {},
        -1,
